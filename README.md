<img width="1167" height="656" alt="image" src="https://github.com/user-attachments/assets/26c96c78-03a9-421d-aae8-069c679ad491" />

<img width="1236" height="693" alt="image" src="https://github.com/user-attachments/assets/139c8058-94bc-4ae0-b71d-4de4265b7eae" />

<img width="1189" height="667" alt="image" src="https://github.com/user-attachments/assets/d2dcf2e9-2332-4a39-8a85-0598ab0c3a9d" />



This is the source code for Animal B-------, a "funny, thought-provoking game about whether animals are b-------" (<a href="https://www.rockpapershotgun.com/animal-bastards-is-funny-thought-provoking-game-about-whether-animals-are-bastards">Rock Paper Shotgun</a>) that has been played by users around the world about 11,000 times. 

Originally it was hosted <a href="https://animalbastards.com/">here</a>, but we need to move it to a cheaper server. Also it only really looks OK on desktop and could use a mobile UI.

